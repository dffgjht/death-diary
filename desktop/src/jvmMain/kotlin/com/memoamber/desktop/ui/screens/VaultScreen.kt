...
@Composable
fun VaultScreen(db: DesktopDatabaseManager, security: DesktopSecurityManager) {
    ...
    var dek by remember { mutableStateOf<ByteArray?>(null) }

    // 获取加密密钥
    LaunchedEffect(Unit) { dek = security.getDataEncryptionKey("current_password") }
    ...
    if (showDialog) {
        AlertDialog(onDismissRequest = { showDialog = false }, title = { Text("添加密码") }, text = {
            ...
            val key = dek ?: return@Button
            val encPwd = security.encryptString(password, key)
