            ...
            AnimatedContent(
                ...
                when (route) {
                    "lock" -> LockScreen(
                        ...
                        onAuthSuccess = { currentRoute.value = "home" }
                    )
                    "home" -> MainScreen(
                        securityManager = securityManager,
                        databaseManager = databaseManager,
                        onLock = { currentRoute.value = "lock" }
                    )
                }
            }
        }
    ...
    Row(modifier = Modifier.fillMaxSize()) {
        NavigationRail(
            ...
            val tabs = listOf(
                ...
                "相册" to Icons.Filled.PhotoLibrary,
                "社区" to Icons.Filled.Forum,
        ...
        Box(modifier = Modifier.fillMaxSize().padding(24.dp)) {
            when (selectedTab) {
                0 -> HomeScreen()
                1 -> DiaryScreen(databaseManager, securityManager)
                2 -> VaultScreen(databaseManager, securityManager)
                3 -> WillScreen(databaseManager, securityManager)
                4 -> GalleryScreen(databaseManager)
                5 -> CommunityScreen()
                6 -> SettingsScreen(securityManager, databaseManager, onLock)
            }
        }
    }
}
